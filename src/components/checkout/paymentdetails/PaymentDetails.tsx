...
const PaymentDetails = () => {
    ...
    return (
        <div className={styles.container}>
            <h2 className="text-center text-2xl md:text-4xl font-semibold">TRANSACTION DETAILS</h2>
            <div className={styles.transactionDetails}>
                        ...
                        <div className="w-full flex flex-col gap-4">
                            ...
                            <div className={styles.card}>
                                <span className={styles.depositSection}>
                                    ...
                                    <h4 className="md:text-2xl font-semibold">{transaction?.fromAmount} {exchange.sending.name}</h4>
                                    <p className="capitalize">{exchange.sending.fullname} Smart Chain Network</p>
                                </span>
                                <span>
                                ...
                                <span className={styles.walletSection}>
                                    <span className="">
                                        <img className="scale-[80%] md:scale-100" src={pocketCoin} alt="eth coin" />
                                    </span>
                                    <span className={styles.copyContainer}>
                                        <h4 className="md:text-2xl font-semibold">{generatedPayment?.to.slice(0, 5)}...{generatedPayment?.to.slice(generatedPayment?.to?.length - 4, generatedPayment?.to?.length)}</h4>
                        ...
                        <div className="w-full flex flex-col gap-4">
                            ...
                            <div className={styles.card}>
                                <span className={styles.depositSection}>
                                    ...
                                    <h4 className="md:text-2xl font-semibold">{transaction?.toAmount} {exchange.receiving.name}</h4>
                                ...
                                <span className={styles.walletSection}>
                                    ...
                                    <span className={styles.copyContainer}>
                                        <h4 className="md:text-2xl font-semibold">{transaction?.receivingWallet.slice(0, 5)}...{transaction?.receivingWallet.slice(transaction?.receivingWallet?.length - 4, transaction?.receivingWallet?.length)}</h4>
                ...
                <div className={styles.timeSection}>
                    <p>Creation Time</p>
                    <p className={styles.highlightedText}>{new Date(transaction?.timeCreated).toLocaleString()}</p>
                </div>
            </div>
        </div>
    );
