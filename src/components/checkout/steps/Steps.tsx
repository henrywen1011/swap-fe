...
const Steps = () => {
    ...
    useEffect(() => {
        const updateStep = () => {
            const now = Date.now();
            const elapsedMinutes = Math.floor((now - transactionCreatedTime) / (1000 * 60)); // Calculate minutes passed
            const newStep = Math.min(Math.floor(elapsedMinutes / 2), stepsArr.length - 1); // Update every 2 minutes, max is the last step
