...
const Instructions = () => {
  ...
  const handleHover = (payload: string) => {
    toggleInstructions((prevState) =>
      prevState.map((instruction: any) => {
  ...
  return (
    <div className={styles.container}>
      {instructions.map((instruction, index) => (
        <div
          onMouseLeave={() => handleHover(instruction.title)}
          onMouseEnter={() => handleHover(instruction.title)}
        ...
        >
          <div className={styles.imageContainer}>
            <img
              ...
              className={`${instruction?.hovered ? "scale-110" : null} ${
                styles.image
              }`}
              alt=""
            />
